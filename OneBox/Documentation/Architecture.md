...
#### CoreVideo
**Purpose**: Video compression and transcoding

**Components**:
- `VideoProcessor` (actor): Thread-safe video operations
- Preset-based compression (AVAssetExportSession)
- Custom bitrate compression (AVAssetWriter)
- Bitrate calculation for target size

**Key Design Decisions**:
- Two-mode architecture: presets vs. custom
- Bitrate formula: `(targetMB * 8e6 / duration) * 0.9`
- H.264 fallback for HEVC-unsupported devices
- Audio track separation for optional muting
...
- System command wrapper (`zip`/`unzip`)
- Encryption detection

**Key Design Decisions**:
- Uses system tools for reliability
- Directory structure preservation
- Error handling for encrypted archives
...
- Videos: Use `AVAssetReader`/`AVAssetWriter` for streaming
...
- **Targets**: 50 images→PDF <12s, 500 MB video compression
